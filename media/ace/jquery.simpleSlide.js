...
function simpleSlideAction(action, rel_no) {
	jQuery(function($) {	
		...
		function move(new_margin, new_swindow_margin, new_stray_margin) {
			...
			else if($.ss_options.move=='fade'){	
				$slice=$('.simpleSlide-slide[rel="' + rel_no + '"]');
				$('.simpleSlide-tray[rel="' + rel_no + '"]').css('margin-left',new_margin);
				$slice.find('img').hide();
				$slice.find('a').hide();
				$slice.find('img').fadeIn('slow');
				$slice.find('a').fadeIn('slow');
				//$slice.find('img').hide();				
				//$slice.find('img')..fadeIn();
				//$slice.fadeIn();
			}
		};
	});
